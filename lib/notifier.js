...
class Notifier {
  constructor () {
    ...
    this.notificationWidth = 450;
    this.queue = [];
    this.startBarSizeEstimate = 80;
  ...
  notify (title, data) {
    ...
    const width = options.width || this.notificationWidth;
    const height = (options.height || this.notificationHeight);
    let windowOptions = {
      width: width,
      height: height + verticalSpace,
      x: size.width - width + (options.xOffset || 0),
    ...
    notificationWindow.webContents.on('did-finish-load', () => {
      try {
        if (notificationWindow.isDestroyed()) {
          console.log('window being destroyed already');
          return;
        }
        console.log('options.css', options.css);
        if (options.css) {
          console.log('inserting css');
          notificationWindow.webContents.insertCSS(options.css);
        }
